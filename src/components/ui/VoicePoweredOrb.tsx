  ...
  `;
    ...
    useEffect(() => {
        ...
        try {
            ...
            let lastTime = 0, currentRot = 0, isMicInit = false;
            if (enableVoiceControl) initMicrophone().then(s => { isMicInit = s; });
            else stopMicrophone();
            ...
            const update = (t: number) => {
                ...
                if (enableVoiceControl && isMicInit) {
                    const vl = analyzeAudio();
                    onVoiceDetected?.(vl > 0.1);
                    if (vl > 0.05) currentRot += dt * (0.3 + vl * maxRotationSpeed * 2);
                    program.uniforms.hover.value = Math.min(vl * 2, 1);
                    program.uniforms.hoverIntensity.value = Math.min(vl * maxHoverIntensity * 0.8, maxHoverIntensity);
                } else {
            ...
            return () => {
                ...
                if (container && glContext?.canvas && container.contains(glContext.canvas)) {
                    try { container.removeChild(glContext.canvas); } catch (e) { }
                }
                stopMicrophone();
    ...
    useEffect(() => {
        if (enableVoiceControl) initMicrophone();
        else stopMicrophone();
    }, [enableVoiceControl]);
